...
def on_playback_audio_frame_before_mixing(agora_local_user, channelId, uid, frame):
    ...
    # uid2 = int(uid)
    # ret,frame_out, flag = vad.Proc(frame.contents.buffer)
    # print(ret, frame_out, flag)
    return 0
...
# 通过传参将参数传进来
# 例如： python examples/example_recv_pcm.py {appid} {token} {channel_id} ./test_data/demo.pcm {userid}
appid = sys.argv[1]
token = sys.argv[2]
