...
#模拟产生的数据，足够多，可以一直播放
def push_file_to_consumer(file, connection:RTCConnection, packnum):
        
        #default to 16k
        bytes_per_frame_in_ms = int(16000 * 2 * 1 // 1000)
        # 10s data if possible
        len = int(bytes_per_frame_in_ms*1000*5)
        frame_buf = bytearray(len)
        read_len = file.readinto(frame_buf)
       
       
        connection.push_audio_pcm_data(frame_buf[:read_len], 16000, 1)
        print("push_file_to_consumer:",read_len, time.time()*1000, "ms")
        file.seek(0)
        
        frame_buf = None   
        pass
async def read_file_to_consumer(pcm_file_path, connection,interval_in_ms, exit_event):
    ...
    with open(pcm_file_path, "rb") as file:
        ...
        push_file_to_consumer(file,connection, 0x7fffffff)    
        file.seek(0)
        ...
        while True:
            ...
            if ret:
                print("push_file_to_consumer:",time.time()*1000, "ms")
                push_file_to_consumer(file,connection, 0x7fffffff)
           
            await asyncio.sleep(interval_in_ms/1000)
    pass
async def do_consume(connection, interval_in_ms, exit_event):
    while True:
...
async def main(exit_event):
    ...
    config.log_path = "./agora_rtc_log/agorasdk.log" # get_log_path_with_filename(os.path.splitext(__file__)[0])
    ...
    connection.register_audio_frame_observer(audio_observer,0, None)
    ...
    """
    produce_task = asyncio.create_task(read_file_to_consumer(pcm_file_path,connection, 100, exit_event))
    consumer_task = asyncio.create_task(do_consume(connection, 40,exit_event))
