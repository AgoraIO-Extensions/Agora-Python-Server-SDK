...
from common.push_audio_pcm_file import push_pcm_data_from_file2
from common.push_video_yuv_file import push_yuv_data_from_file2
...
class RTCProcessIMPL(RTCBaseProcess):
    ...
    async def send(self, sample_options: ExampleOptions, pcm_data_sender, yuv_data_sender):
        pcm_task = asyncio.create_task(push_pcm_data_from_file2(sample_options.sample_rate, sample_options.num_of_channels, pcm_data_sender, sample_options.audio_file))
        yuv_task = asyncio.create_task(push_yuv_data_from_file2(sample_options.width, sample_options.height, sample_options.fps, yuv_data_sender, sample_options.video_file))
        await pcm_task
        await yuv_task
