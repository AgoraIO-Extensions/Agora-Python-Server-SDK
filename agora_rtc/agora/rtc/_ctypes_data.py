...
class VadConfigInner(ctypes.Structure):
    _fields_ = [
        ("fftSz", ctypes.c_int),
        ("hopSz", ctypes.c_int),
    ...
    def get(self) -> VadConfig:
        return VadConfig(
            fftSz=self.fftSz,
            hopSz=self.hopSz,
            anaWindowSz=self.anaWindowSz,
            frqInputAvailableFlag=self.frqInputAvailableFlag,
            useCVersionAIModule=self.useCVersionAIModule,
            voiceProbThr=self.voiceProbThr,
            rmsThr=self.rmsThr,
            jointThr=self.jointThr,
            aggressive=self.aggressive,
            startRecognizeCount=self.startRecognizeCount,
            stopRecognizeCount=self.stopRecognizeCount,
            preStartRecognizeCount=self.preStartRecognizeCount,
            activePercent=self.activePercent,
            inactivePercent=self.inactivePercent
        )
    ...
    def create(config: VadConfig) -> 'VadConfigInner':
        return VadConfigInner(
            ...
            config.hopSz,
            config.anaWindowSz,
            config.frqInputAvailableFlag,
            config.useCVersionAIModule,
            config.voiceProbThr,
            config.rmsThr,
            config.jointThr,
            config.aggressive,
            config.startRecognizeCount,
            config.stopRecognizeCount,
            config.preStartRecognizeCount,
            config.activePercent,
            config.inactivePercent
        )
